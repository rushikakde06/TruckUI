...
import { useTelemetryStream, telemetryStreamService } from '../../services/telemetryStream';
import { useGeolocation } from '../../services/geolocation';
import { MapPin, AlertCircle, TrendingUp, Navigation, Zap } from 'lucide-react';
...
export function LiveMapPage() {
  ...
  const { data: selectedTelemetry } = useTelemetryStream(selectedVehicle, token);
  ...
  useEffect(() => {
    ...
    const fetchVehicles = async () => {
      try {
        const data = await vehiclesAPI.listVehicles(token, 0, 50);
        setVehicles(data);
        if (data.length > 0) {
          setSelectedVehicle(data[0].id);
          setMapCenter({
            lat: data[0].current_latitude || 40.7128,
            lon: data[0].current_longitude || -74.006,
          });
        }
        setLoading(false);
      } catch (error) {
  ...
  // Start telemetry polling for selected vehicle
  useEffect(() => {
    if (!selectedVehicle || !token) return;

    telemetryStreamService.startPolling(selectedVehicle, token, 3000);
  ...
  useEffect(() => {
    if (selectedTelemetry) {
      setMapCenter({
        lat: selectedTelemetry.latitude,
        lon: selectedTelemetry.longitude,
      });
    }
  }, [selectedTelemetry]);

  // Simple map projection function
  const projectToMap = (lat: number, lon: number) => {
    ...
    const x = ((lon - mapCenter.lon) / lonRange) * (mapWidth / 2) + mapWidth / 2;
    const y = ((mapCenter.lat - lat) / latRange) * (mapHeight / 2) + mapHeight / 2;
    ...
    <div style={{ padding: '0 24px 32px', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: '#64748B', fontSize: 14 }}>
          Real-time vehicle tracking with AI insights
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            {userLocation && (
              <>
                <circle
                  cx={projectToMap(userLocation.latitude, userLocation.longitude).x}
                  cy={projectToMap(userLocation.latitude, userLocation.longitude).y}
                  r={8}
                  fill="#10B981"
                  opacity={0.3}
                />
            ...
            {vehicles.map((vehicle, idx) => {
              const lat = vehicle.current_latitude || 40.7128 + Math.random() * 0.1;
              const lon = vehicle.current_longitude || -74.006 + Math.random() * 0.1;
              const pos = projectToMap(lat, lon);

              const isSelected = vehicle.id === selectedVehicle;
              const riskScore = selectedTelemetry?.risk_score || 0;
              const color =
                riskScore > 70
                  ? '#DC2626'
                  : riskScore > 40
          ...
          {selectedTelemetry && (
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                ...
                >
                  <div style={{ color: '#94A3B8', fontSize: 10 }}>Speed</div>
                ...
                >
                  <div style={{ color: '#94A3B8', fontSize: 10 }}>Temp</div>
                ...
                <div
                  style={{
                    background:
                      selectedTelemetry.risk_score > 70
                        ? 'rgba(220,38,38,0.1)'
                        : 'rgba(245,158,11,0.1)',
                    padding: 8,
                    borderRadius: 6,
                ...
                >
                  <div style={{ color: '#94A3B8', fontSize: 10 }}>Risk</div>
                  <div
                    style={{
                      color:
                        selectedTelemetry.risk_score > 70 ? '#DC2626' : '#F59E0B',
                      fontSize: 14,
                      fontWeight: 600,
                    }}
                  >
                    {selectedTelemetry.risk_score.toFixed(1)}/100
                  </div>
                </div>
                ...
                >
                  <div style={{ color: '#94A3B8', fontSize: 10 }}>Confidence</div>
                  <div style={{ color: '#22C55E', fontSize: 14, fontWeight: 600 }}>
                    {(selectedTelemetry.ai_confidence * 100).toFixed(0)}%
                  </div>
                </div>
              </div>
              ...
              >
                Updated: {new Date(selectedTelemetry.last_update).toLocaleTimeString()}
              </div>
            </div>
          )}
        </div>
